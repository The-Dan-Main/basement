...
2. Run the SQL in `supabase/migrations/` in order (`001_init.sql`, then `002_locale.sql` if you already applied an older `001`). The landing page also copies this SQL when keys are missing.
...
- `src/routes/app` — lists, household, settings (auth required)
